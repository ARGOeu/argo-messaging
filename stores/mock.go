...
func (mk *MockStore) Initialize() {
	...
	qsub1 := QSub{"ARGO", "sub1", "topic1", 0, 0, "", "", 1, 10, "linear", 300}
	qsub2 := QSub{"ARGO", "sub2", "topic2", 0, 0, "", "", 1, 10, "linear", 300}
	qsub3 := QSub{"ARGO", "sub3", "topic3", 0, 0, "", "", 1, 10, "linear", 300}
	qsub4 := QSub{"ARGO", "sub4", "topic4", 0, 0, "", "endpoint.foo", 1, 10, "linear", 300}
...
func (mk *MockStore) InsertSub(project string, name string, topic string, offset int64, ack int, push string, pushMaxMsg int, rPolicy string, rPeriod int) error {
