...
func (suite *AuthTestSuite) TestFindUserRegistration() {
	...
	ur1, e1 := FindUserRegistration("uratkn-1", "pending", store)
...
func (suite *AuthTestSuite) TestUpdateUserRegistration() {
	...
	e1 := UpdateUserRegistration("uratkn-1", AcceptedRegistrationStatus, "uuid1", m, store)
	ur1, _ := FindUserRegistration("uratkn-1", "accepted", store)
	expur1 := UserRegistration{
		UUID:            "ur-uuid1",
		Name:            "urname",
		FirstName:       "urfname",
...
func (suite *AuthTestSuite) TestFindUserRegistrations() {
	...
	store := stores.NewMockStore("", "")
	ur1, e1 := FindUserRegistrations(store)
